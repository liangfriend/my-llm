...
exports.fillNote = fillNote;
exports.generateMelody = generateMelody;
const probabilityModel_1 = require("./probabilityModel");
...
function normalizeMidiBound(value) {
    const num = Math.round(Number(value));
    if (!Number.isFinite(num))
        return null;
    return Math.min(128, Math.max(1, num));
}
function resolveMidiRange(minMidi, maxMidi) {
    var _a, _b;
    let min = (_a = normalizeMidiBound(minMidi)) !== null && _a !== void 0 ? _a : 1;
    let max = (_b = normalizeMidiBound(maxMidi)) !== null && _b !== void 0 ? _b : 128;
    if (min > max) {
        [min, max] = [max, min];
    }
    return { min, max };
...
// 检索note,使其保持在midi范围内
function keepMidiInRange(note, melody, range) {
    if (note.rest)
        return note;
    const { min, max } = range;
    ...
    const recommended = (0, note_1.recommendMidiReplace)(melody.length, tempNotes) || [];
...
// 填充音符
function fillNote(seedNote, lyric, model, prevMidi, options = {}) {
    const rules = options.chronaxieRules || (0, note_1.resolveChronaxieRules)(undefined, undefined);
    const note = { ...seedNote };
    if (lyric !== undefined)
        note.lyrics = lyric;
    const sampledMidi = model.sampleMidi(prevMidi, options.midiRange);
    const midi = (0, note_1.clampMidi)(note.midi);
    note.midi = midi !== null && midi !== void 0 ? midi : sampledMidi;
    // 是否有时值
    const hasChronaxieOverride = note.chronaxie !== undefined;
    // 有时值使用时值，没有时值获取时值
    const sampledChronaxie = hasChronaxieOverride
        ? (0, note_1.normalizeChronaxie)(note.chronaxie, rules)
        : model.sampleChronaxie(prevMidi);
    // 约束时值
    let chronaxie = (0, note_1.normalizeChronaxie)(sampledChronaxie, rules);
    ...
    if (!hasChronaxieOverride && options.targetChronaxie !== undefined && isPositiveNumber(options.targetChronaxie)) {
        const target = (0, note_1.normalizeChronaxie)(options.targetChronaxie, rules);
        chronaxie = (0, note_1.normalizeChronaxie)(target + Math.round((chronaxie - target) / 2), rules);
    }
    // 传入数据没有时值且当前时值超出累加器的时候的情况下，对目标时值和获取的时值取最小值
    if (!hasChronaxieOverride && options.overExpected !== undefined) {
        // TODO 目前，即使没有剩余时值，也会返回128
        const boundedTarget = (0, note_1.normalizeChronaxie)(options.targetChronaxie, rules);
        chronaxie = Math.min(chronaxie, boundedTarget);
    }
    note.chronaxie = chronaxie;
    return (0, note_1.sanitizeNote)(note, undefined, rules);
}
// 添加休止符
function addRestDuration(duration, melody, rules) {
    let remaining = Math.round(duration);
    let added = 0;
    while (remaining >= rules.minChronaxie) {
        const chunk = Math.min(remaining, rules.maxChronaxie);
        const restNote = (0, note_1.sanitizeNote)({ rest: true, chronaxie: chunk }, undefined, rules);
        melody.push(restNote);
        added += restNote.chronaxie;
        remaining -= restNote.chronaxie;
        if (restNote.chronaxie <= 0)
            break;
    }
    return added;
}
// 如果当前时值还是大于总时值，将剩余时值并入最后一个音符
function adjustMelodyToTotalChronaxie(melody, targetTotal, rules) {
    ...
    const mergedChronaxie = (0, note_1.normalizeChronaxie)(melody[lastIdx].chronaxie + diff, rules);
...
// 旋律生成入口函数
function generateMelody({ text, seedMelody, length, params, totalChronaxie, minMidi, maxMidi, minChronaxie, minChronaxieInterval, }) {
    // 加载训练样本数据
    const data = (0, storage_1.loadTrainingData)();
    // 创建模型
    const model = (0, probabilityModel_1.buildProbabilityModel)(data.examples, params || {});
    // 歌词数组
    const chars = typeof text === 'string' ? Array.from(text) : [];
    // 种子数据
    const seed = Array.isArray(seedMelody) ? seedMelody : [];
    // 目标长度
    const targetLength = (0, note_1.resolveTargetLength)(length, chars.length, seed.length);
    const melody = [];
    let prevMidi = null;
    const warnings = [];
    // 约束总时值
    ...
    const chronaxieRules = (0, note_1.resolveChronaxieRules)(minChronaxie, minChronaxieInterval);
    // 每个音符的标准时值
    const standardChronaxie = totalChronaxieNumber !== null && targetLength > 0
    ...
    if (totalChronaxieNumber !== null && targetLength > seed.length && seed.length) {
        const seedChronaxieTotal = seed.reduce((sum, note) => {
            ...
            return Number.isFinite(val) ? sum + (0, note_1.normalizeChronaxie)(val, chronaxieRules) : sum;
        }, 0);
    ...
    if (totalChronaxieNumber !== null && accumulatedChronaxie < totalChronaxieNumber) {
        const added = addRestDuration(totalChronaxieNumber - accumulatedChronaxie, melody, chronaxieRules);
        accumulatedChronaxie += added;
    }
    // 如果休止富添加不了，说明还剩下很小的时值，并入最后一个音符
    const finalMelody = totalChronaxieNumber !== null && melody.length
