...
- 从 `training-data.json` 读取样本，按参数相似度加权统计全局直方图和相邻音高转移概率。
- 生成时优先使用前一音的转移分布；若无数据则回退到全局分布，再无则回退到中音区。
- chronaxie 按直方图采样，缺失时默认 4。
- 参数相似度：相同字符串提升权重；数值按距离衰减；缺失参数轻度惩罚。

扩展建议
--------

- 增加节奏、调式等特征到 `params`，可提升相似度加权效果。
- 在 `training-data.json` 中持续追加样本，模型会实时利用新样本，无需重启。

Project structure
-----------------

- `index.js`: Express å…¥å£åŒ API è·¯ç”±ç®¡æŽ§ã€‚
- `lib/generator.js`: æ—‹å¾‹ç”Ÿæˆæµç¨‹ã€‚
- `lib/probabilityModel.js`: æ¨¡åž‹åŠ æƒå’Œé‡‡æ ·ç®—æ³•ã€‚
- `lib/note.js`: midi/chronaxie æ ‡å‡†åŒ–åŒéŸ³ç¬¦æ¸…æ´åŒ–ã€‚
- `lib/storage.js`: æ•°æ®è¯»å†™ã€‚
- `lib/validation.js`: è¾“å…¥éªŒè¯ã€‚
Hot reload
----------

- Dev: `npm run dev` uses `nodemon` to restart automatically when files change.
- `npm start` keeps the non-watch mode if you prefer a static process; default port remains 3000.
