import { GenerationState, TrainingExample } from '../type';
import { calcParamsSimilarityWeight } from './weight';
...
/** 保留 weight >= 中位数的项 */
export function filterByMedianWeight<T extends { weight: number }>(items: T[]): T[] {
 ...
 * 防止权重全相等时死循环。
 */
export function medianFilterUntil<T extends { weight: number }>(
  items: T[],
  maxCount: number,
): T[] {
  let current = [...items];
 ...
 */
export function isSampleWithinRequestMidiRange(
  example: TrainingExample,
  state: GenerationState,
): boolean {
  const hasRequestMin = state.minMidi !== null;
  const hasRequestMax = state.maxMidi !== null;
  if (!hasRequestMin && !hasRequestMax) return true;

  const sampleMin = example.minMidi;
  const sampleMax = example.maxMidi;
  if (sampleMin === undefined && sampleMax === undefined) return true;

  const resolvedSampleMin = sampleMin ?? 1;
  const resolvedSampleMax = sampleMax ?? 128;

  if (hasRequestMin && state.minMidi! > resolvedSampleMin) return false;
  if (hasRequestMax && state.maxMidi! < resolvedSampleMax) return false;
  return true;
}

/** 第 2 步：单个样本的 params 权重（当前唯一权重维度） */
export function calcSampleParamsWeight(
  example: TrainingExample,
  state: GenerationState,
): number {
  if (!state.params) {
  ...
  return calcParamsSimilarityWeight(example.params || {}, state.params);
}
 ...
 */
export function filterSamplesStep2(state: GenerationState, examples: TrainingExample[]): void {
  // 在样本范围内
  ...
  const filteredSamples: TrainingExample[] = [];
