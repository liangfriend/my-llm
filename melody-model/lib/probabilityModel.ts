import { clampMidi, normalizeChronaxie, RawNote } from './note';
...
export function buildProbabilityModel(
  ...
  function sampleMidi(prevMidi: number | null): number {
    ...
    const fallbackChronaxie = chronaxieCounts.size ? sampleFromMap(chronaxieCounts, 128) ?? 128 : 128;
    // 通过概率计算拿到时值
  function sampleChronaxie(prevMidi: number | null): number {
    ...
    if (chronaxieCounts.size) return sampleFromMap(chronaxieCounts, 4) ?? 4;
    return 4;
  }
