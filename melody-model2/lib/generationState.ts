...
export function isChronaxieAllowedForState(state: GenerationState, chronaxie: number): boolean {
  if (state.minChronaxie === null && state.minChronaxieInterval === null) return true;
  const rules = resolveChronaxieRules(state.minChronaxie, state.minChronaxieInterval);
  if (chronaxie < rules.minChronaxie) return false;
...
export function getEffectiveMinChronaxie(state: GenerationState): number {
  if (state.minChronaxie !== null) return state.minChronaxie;
  return resolveChronaxieRules(undefined, undefined).minChronaxie;
}
